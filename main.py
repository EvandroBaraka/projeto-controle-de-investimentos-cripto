...
titulo = tk.CTkLabel(janela, font=('', 20), text='Controle de Criptos')
label_consulta = tk.CTkLabel(janela, text='Consulta Cripto')
label_cotacao = tk.CTkLabel(janela, text='Cotação Atual')
select_moeda = tk.CTkOptionMenu(janela, width=170, values=lista_criptos, command=exibir_cotacao)
valor_moeda = tk.CTkLabel(janela, text='R$ 0,00')
label_compra = tk.CTkLabel(janela, text='Compra de Moeda')
...
janela_dados_investimentos = tk.CTkScrollableFrame(janela, width=550, height=100)
label_dados_investimentos = tk.CTkLabel(janela_dados_investimentos, text=ler_arquivo_investimentos(), justify='left')
...
janela_dados_investimentos.grid(column=0, row=6, pady=20, padx=20, columnspan=2)
